...
print(f'PSD: {np.mean(used_time):.4f} seconds per threshold') # 0.9205s
...
for drac_threshold in tqdm(np.round(np.arange(0.05,5.,0.05),2), desc='DRAC'):
    ...
    demonstration.append(meta)
demonstration = pd.concat(demonstration).reset_index(drop=True)
print(f'DRAC: {np.mean(used_time):.4f} seconds per threshold') # 0.9852s
...
for ttc_threshold in tqdm(np.round(np.arange(0.2,20.,0.2),1), desc='TTC'):
    ...
    demonstration.append(meta)
demonstration = pd.concat(demonstration).reset_index(drop=True)
print(f'TTC: {np.mean(used_time):.4f} seconds per threshold') # 1.3176s
...
for n in tqdm(n_list, desc='Unified'):
    ...
    demonstration.append(meta)
demonstration = pd.concat(demonstration).reset_index(drop=True)
print(f'Unified: {(pre_computation_time+np.mean(used_time)):.4f} seconds per threshold') # 4.6558s)
