...
This study is being prepared for submission. A preprint is available at [arXiv](https://arxiv.org/abs/2407.10959).

This study is still under active refinement. Any questions or suggestions are welcome.
...
## Abstract
This study proposes a unified theory and statistical learning approach for traffic conflict detection, addressing the long-existing call for a consistent and comprehensive methodology to evaluate the collision risk emerged in road user interactions. The proposed theory describes context-dependent relationships between proximity, conflict intensity, and conflict probability, where conflicts are extreme events in daily interactions; and the statistical learning approach integrates three tasks to capture the relationships: representing interaction context from selected observables, inferring proximity distributions in different contexts, and relating conflict intensity with conflict probability through extreme value theory. Experiments using real-world trajectory data are conducted in this study, where a unified metric is trained on lane-changing interactions on German highways and applied to near-crash events from the 100-Car Naturalistic Driving Study in the U.S. Results of the experiments demonstrate that the unified metric provides effective collision warnings,  generalises across distinct datasets and traffic environments, covers a broad range of conflicts, and delivers a long-tailed distribution of conflict intensity. Jointly, the theory and learning approach provide an adaptable and explainable methodology to conflict detection among different road users and across various interaction scenarios. This contributes to enhanced safety assessment of traffic infrastructures, more effective collision warning systems for autonomous driving, and a deeper understanding of road user behaviour in different traffic conditions, promising a reduction in accident rates and an improvement in overall traffic safety.
